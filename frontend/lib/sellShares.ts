...
import { CONTRACTS, PROPERTY_TOKENS } from "../../src/lib/contracts";
import PropertySellABI from "../../src/lib/abis/PropertySell.json";
import PropertyTokenABI from "../../src/lib/abis/PropertyToken.json";
...
export async function sellShares(
  ...
  try {
    ...
    if (allowance < tokenAmount) {
      const approveTx = await propertyToken.approve(
        CONTRACTS.propertySell,
        tokenAmount
      );
    ...
    const tx = await propertySell.sellShares(
      propertyTokenAddress,
      tokenAmount,
      usdcRaw
    );
